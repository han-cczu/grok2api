...
def _image_content(value: str, response_field: str) -> str:
    """Build markdown image string from an image output value."""
    if not value:
        return ""
    if response_field == "url" or value.startswith("http"):
        return f"![image]({value})"
    return f"![image](data:image/png;base64,{value})"


async def _wrap_image_stream_as_chat(
    image_stream: AsyncGenerator[str, None],
    model: str,
    response_field: str,
) -> AsyncGenerator[str, None]:
    """Convert image SSE stream into OpenAI chat.completion.chunk format."""
    response_id = f"chatcmpl-{uuid.uuid4().hex[:24]}"
    created = int(time.time())

    def _chunk(content: str = "", role: str = None, finish: str = None) -> str:
        delta: Dict[str, Any] = {}
        if role:
            delta["role"] = role
            delta["content"] = ""
    ...
    async for sse_event in image_stream:
        event_type = ""
        data_json = ""
        ...
        if event_type == "image_generation.completed":
            output = data.get(response_field, "")
            md = _image_content(output, response_field)
            if md:
                yield _chunk(f"{md}\n")
        elif event_type == "error":
            err = data.get("error", {})
            msg = err.get("message", "Image generation failed") if isinstance(err, dict) else str(err)
            yield _chunk(f"Error: {msg}\n")
...
def _build_image_chat_response(
    ...
    """Build an OpenAI chat.completion response from image results."""
    parts = [_image_content(img, response_field) for img in images]
    ...
    if model_info and model_info.is_image_edit:
        ...
        if result.stream:
            return StreamingResponse(
                _wrap_image_stream_as_chat(result.data, request.model, response_field),
